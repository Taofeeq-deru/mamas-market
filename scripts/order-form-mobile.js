...
function saveItem(shopItem) {
  ...
  item.amountPerItem = item.amount / item.weight;
...
function addMore() {
  ...
  <div class="form-group col-2">
    <div class="bg-white d-flex rounded-lg pr-2">
      <!--prettier-ignore-->
      <select name="list[${id}][unit]" id="unit${id}" class="form-control unit border-0 w-50 h-50 my-1">
        <option value="kg">kg</option>
        <option value="">.</option>
      </select>
      <!--prettier-ignore-->
      <input type="number" name="list[${id}][weight]" id="weight${id}" value="1" class="form-control weight bg-grey w-50 h-50 my-1" />
    </div>
  </div>
  <div class="form-group col-8">
    <!--prettier-ignore-->
    <select name="list[${id}][product]" id="list${id}" class="form-control select-list">
    ...
    <!--prettier-ignore-->
    <input type="text" name="list[${id}][product-preference]" id="product-preference${id}" class="form-control my-2 list-preference" placeholder="Do you have any preference? Kindly type it here" />
  </div>
  <div class="form-group col-2">
    <div class="d-flex">
      <i class="fa fa-naira mt-2 fa-lg"></i>
      <!--prettier-ignore-->
      <input type="text" name="list[${id}][product-amount]" id="product-amount${id}" class="form-control bg-white font-weight-bold text-center product-amount" placeholder="Amount" value="" readonly />
    </div>
...
function showProducts(value) {
  ...
  <div class="form-row pb-3 pt-3 border-bottom-grey">
    ...
    <div class="form-group col-2 d-flex flex-row justify-content-around">
      <!--click to increase-->
      <!--prettier-ignore-->
      <p class="fa-lg text-decoration-none text-dark mt-small subtract" onclick="decreaseQty(event)">-</p>
      <!--product quantity-->
      <!--prettier-ignore-->
      <input type="text" name="quantity${uuid}" id="quantity${uuid}"
        class="form-control w-75 bg-white quantity border-right-0 rounded-left fa-show-md py-sm-1 px-1 px-sm-2" 
        value="${value.weight}" disabled />
      <!--click to decrease-->
      <!--prettier-ignore-->
      <p class="fa-lg text-decoration-none text-dark mt-small add" onclick="increaseQty(event)">+</p>
    </div>
    <!--prettier-ignore-->
    <div class="form-group col-3 d-flex flex-row justify-content-center">
      <!--product price per item-->
      <i class="fa fa-naira mt-small fa-lg"></i>
      <!--prettier-ignore-->
      <input type="text" name="pricePerItem${uuid}" id="pricePerItem${uuid}"
        class="form-control w-75 bg-white pricePerItem text-center fa-show-md py-sm-1 px-1 px-sm-2"
        value="${value.amountPerItem}" disabled />
    </div>
    <!--prettier-ignore-->
    <div class="form-group col-3 d-flex flex-row justify-content-center">
      <!--product price-->
      ...
      <input type="text" name="price${uuid}" id="price${uuid}"
        class="form-control w-75 bg-white price text-center fa-show-md py-sm-1 px-1 px-sm-2"
        value="${value.amount}" disabled />
    </div>
    <!--prettier-ignore-->
    <div class="form-group col-1 d-flex flex-row justify-content-start">
      <!--click to remove product-->
      <!--prettier-ignore-->
      <p class="fa-md text-decoration-none text-dark mt-sm-2 remove text-danger" onclick="remove(event)">&times;</p>
    </div>
  </div>
</li>`;
